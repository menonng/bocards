...
function makeDeck(rng, ownerTag, ratio) {
    ...
    while (instances.length < DECK_SIZE) {
        ...
        if (def.rarity === "legendary" && usedLegendary.has(def.id)) {
            ...
            if (alt.length > 0)
                def = alt[Math.floor(rng() * alt.length)];
            // alt가 비어있으면(극단적 케이스) 중복을 그대로 허용한다.
        }
        if (def.rarity === "legendary")
            usedLegendary.add(def.id);
...
export function startGame(action) {
    ...
    // 그대로 사용 — 테스트/디버그 용도)
    const stageCardIds = action.stageCardIds && action.stageCardIds.length > 0
        ? action.stageCardIds
...
function runIntroAndDraw(state) {
    ...
    if (result.phase !== "gameover") {
        for (const idx of [0, 1]) {
            if (result.players[idx].hand.length === 0) {
                ...
                result = withLog(result, `${result.players[idx].name}의 손패가 바닥나 ${HAND_REFILL_ON_EMPTY}장을 더 뽑았습니다.`);
            }
        }
    }
