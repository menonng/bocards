...
function orbitRings(color) {
    const rings = [70, 54, 38, 22]
        .map((r, i) => `<circle cx="90" cy="70" r="${r}" fill="none" stroke="${withAlpha(color, 0.55 - i * 0.1)}" stroke-width="6"/>`)
        .join("");
...
function scatterShards(color, seed) {
    let s = seed || 1;
    const rand = () => {
...
export function producerBadgeDataUri(color, initials) {
...
//# sourceMappingURL=cardArt.js.map