...
export interface GameHandlers {
  onPlayCard: (playerIndex: 0 | 1, instanceId: string) => void;
  onEndTurn: (playerIndex: 0 | 1) => void;
  onPassReaction: (playerIndex: 0 | 1) => void;
...
export function renderSetup(onStart: (opts: SetupOptions) => void): HTMLElement {
  const stageOptions = STAGE_CARDS.map(
    (s, i) => `
  ...
  root.querySelector("#start-btn")!.addEventListener("click", () => {
    const stageId =
      (root.querySelector('input[name="stage"]:checked') as HTMLInputElement)?.value ??
      STAGE_CARDS[0].id;
    const mode = (root.querySelector("#mode-select") as HTMLSelectElement).value as GameMode;
    const p0 = (root.querySelector("#p0-name") as HTMLInputElement).value.trim();
    const p1 = (root.querySelector("#p1-name") as HTMLInputElement).value.trim();
...
export function renderHandoff(
  ...
  const reasonText =
    state.phase === "reaction" ? "코러스 카드로 반응할 차례입니다" : "당신의 턴입니다";
  const root = el(`
...
function renderCard(
  ...
  `);
  if (onClick && playable) node.addEventListener("click", onClick);
  return node;
}

// ── 게임 보드 ────────────────────────────────────────────────

export function renderGame(state: GameState, handlers: GameHandlers): HTMLElement {
  if (state.phase === "gameover") return renderGameOver(state, handlers);

  const active = state.activePlayerIndex;
  const isReaction = state.phase === "reaction";
  const viewerIndex: 0 | 1 = isReaction ? (active === 0 ? 1 : 0) : active;
  const opponentIndex: 0 | 1 = viewerIndex === 0 ? 1 : 0;
  ...
  root.querySelector("#panel-opponent")!.replaceWith(
    renderPlayerPanel(state, opponentIndex, "상대"),
  );
  ...
  for (const card of player.hand) {
    ...
    handArea.appendChild(
      renderCard(def, card, check.ok, check.reason, () =>
        handlers.onPlayCard(viewerIndex, card.instanceId),
      ),
    );
  }
  ...
  if (isReaction) {
    controls.appendChild(
      el(
        `<button id="pass-btn" class="secondary-btn">패스 (반응하지 않음)</button>`,
      ),
    );
    controls.querySelector("#pass-btn")!.addEventListener("click", () =>
      handlers.onPassReaction(viewerIndex),
    );
  } else {
    controls.appendChild(
      el(
        `<div class="pp-info">남은 메인 발동: ${player.mainPlaysRemaining} · 재생 포인트: ${player.pp}/${player.ppMax}</div>`,
      ),
    );
    controls.appendChild(el(`<button id="end-turn-btn" class="primary-btn">턴 종료</button>`));
    controls.querySelector("#end-turn-btn")!.addEventListener("click", () =>
      handlers.onEndTurn(viewerIndex),
    );
  }
...
function renderPlayerPanel(state: GameState, index: 0 | 1, label: string): HTMLElement {
...
function renderRevealModal(state: GameState, handlers: GameHandlers): HTMLElement {
...
function renderGameOver(state: GameState, handlers: GameHandlers): HTMLElement {
  const [p0, p1] = state.players;
  const winnerText =
    state.winnerIndex === "draw"
      ? "무승부!"
      : `${state.players[state.winnerIndex as 0 | 1].name} 승리!`;
  const root = el(`
